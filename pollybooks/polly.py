...
def run_on_file(
    infile: Path, outdir: Path, voice="Salli", paragraph_space_sec: int = 2
):
    ...
    for line in track(list_of_paragraphs, description=f"Converting {infile} to audio"):
        ...
        rendered = (
            '<speak><amazon:effect name="drc">'
            + line.strip()
            + f'<break time="{paragraph_space_sec}s"/></amazon:effect></speak>'
        )
...
def run_on_file_or_dir(
    infile_or_dir: Path, outdir: Path, voice="Salli", paragraph_space_sec: int = 2
):
    """
    ...
    if infile_or_dir.is_file():
        return run_on_file(
            infile_or_dir, outdir, voice=voice, paragraph_space_sec=paragraph_space_sec
        )
        ...
        for file in infile_or_dir.iterdir():
            run_on_file(
                file, outdir, voice=voice, paragraph_space_sec=paragraph_space_sec
            )
    else:
