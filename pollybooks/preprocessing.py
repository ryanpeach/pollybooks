...
def split_text_into_n_token_strings(text: str, max_tokens: int = MAX_TOKENS, n_chars_per_token: int = N_CHARS_PER_TOKEN) -> List[str]:
    """
    ...
    for line in lines:
        if count_tokens((current_substring + '\n' + line).strip(), n_chars_per_token=n_chars_per_token) >= max_tokens:
...
def convert_via_gpt(infile: Path, outpath: Path):
    ...
    chunks = split_text_into_n_token_strings(text, max_tokens=MAX_TOKENS - conversion_prompt_tokens)
...
if __name__=="__main__":
    convert_via_gpt(Path('./examples/preprocessing/frankenstein/source.html'), Path('./examples/preprocessing/frankenstein/'))
